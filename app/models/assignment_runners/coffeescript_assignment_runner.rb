class CoffeescriptAssignmentRunner
  ...
  def run!
    ...
    f = File.new("#{test_filename}.coffee", "w+")
    f.puts(@assignment_submission.code)
    f.puts
    f.puts(@assignment.code)
    f.close

    output = %x{ ~/node/node_modules/jasmine-node/bin/jasmine-node --coffee #{test_filename}.coffee }

    # save returned status code and output
    result = [$? == 0 ? 0 : 1, output]

    %x{ rm #{test_filename}.coffee }
